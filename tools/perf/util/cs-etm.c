					    ...
					    u64 addr, u64 period)
{
	...
	/* Sample the callchain until a valid kernel start address */
	if (etm->synth_opts.callchain && tidq->kernel_start) {
				       ...
				       struct perf_session *session)
{
	...
	if (etm->synth_opts.calls)
		etm->branches_filter |= PERF_IP_FLAG_CALL | PERF_IP_FLAG_TRACE_END;
