...
{
	u64 op = PERF_MEM_OP_NA, sub_op = 0;
	int l;

	if (mem_info) {
		op = mem_info__const_data_src(mem_info)->mem_op;
		sub_op = mem_info__const_data_src(mem_info)->mem_op_ext;
	}
	...
	if (sub_op & PERF_MEM_EXT_OP_MTE_TAG)
		l = scnprintf(out, sz, " MTE");
	else if (sub_op & PERF_MEM_EXT_OP_NESTED_VIRT)
		l = scnprintf(out, sz, " NV");
	else if (sub_op & PERF_MEM_EXT_OP_MEMCPY)
		l = scnprintf(out, sz, " MEMCPY");
	else if (sub_op & PERF_MEM_EXT_OP_MEMSET)
		l = scnprintf(out, sz, " MEMSET");
	else if (sub_op & PERF_MEM_EXT_OP_SIMD)
		l = scnprintf(out, sz, " SIMD");
