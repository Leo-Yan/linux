...
Decode branch history from AUX samples (prototype)
--------------------------------------------------
...
samples. The prototype augments samples in ``perf script`` and ``perf report``;
synthetic instruction events and saving reconstructed callchains with
``perf inject`` are outside its scope.

This prototype supports unformatted, per-CPU TRBE trace. Each sample selects
...
Formatted sinks and guest samples are outside this prototype's scope.
