...
static struct coresight_path *coresight_cpu_get_active_path(enum cs_mode mode)
{
	...
	/*
	 * It is expected to run either in atomic context or with the CPUs
	 * lock held, so it cannot be preempted to disable the path.
	 * Here returns the active path pointer without concern that its
	 * state may change, and it can be safely used by the caller.
	 */
