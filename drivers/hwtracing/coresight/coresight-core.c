			    ...
			    struct coresight_path *path)
{
	 ...
	 * on the CPU instead. Otherwise, there is a window where the source is
	 * enabled but the path pointer is not yet set, which can cause issues
	 * for CPU PM notifiers.
	 */
...
/*
 * coresight_disable_path_from_to: Disable components in the given path from
 * @from to @to (inclusive) in the list. If both @from and @to are NULL, all
 * the components, except the SOURCE are disabled.
 */
...
static bool coresight_get_ref(struct coresight_device *csdev)
{
	...
	/* Make sure csdev and parent device can't go away */
	get_device(&csdev->dev);
	get_device(parent);
...
static struct coresight_path *coresight_cpu_get_active_path(enum cs_mode mode)
{
	...
	/*
	 * It is expected to run either in atomic context or with the CPU lock
	 * held for sysfs mode, so it cannot be preempted to disable the path.
	 * Here returns the active path pointer without concern that its state
	 * may change, and it can be safely used by the caller.
	 */
