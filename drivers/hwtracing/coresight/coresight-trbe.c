...
VISIBLE_IF_KUNIT atomic_t trbe_trigger_mode_bypass = ATOMIC_INIT(0);
...
unsigned long __trbe_normal_offset(struct perf_output_handle *handle)
{
	...
	if (!atomic_read(&trbe_trigger_mode_bypass))
		goto out;
...
static void arm_trbe_enable_cpu(void *info)
{
	...
	if (trbe_trigger_mode_need_bypass(cpudata))
		atomic_fetch_inc(&trbe_trigger_mode_bypass);
}
...
static void arm_trbe_disable_cpu(void *info)
{
	...
	if (trbe_trigger_mode_need_bypass(cpudata))
		atomic_fetch_dec(&trbe_trigger_mode_bypass);
