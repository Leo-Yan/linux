...
static unsigned long __trbe_normal_limit(struct perf_output_handle *handle)
{
	 ...
	 * trbe_base		limit		trbe_base + nr_pages
	 */
	if (static_branch_unlikely(&trbe_trigger_mode_bypass_key) &&
	    (handle->wakeup < (handle->head + handle->size) && head <= wakeup))
...
static u64 __trbe_normal_count(struct perf_output_handle *handle)
{
	struct trbe_buf *buf = etm_perf_sink_config(handle);
	struct trbe_cpudata *cpudata = buf->cpudata;
...
static int trbe_normal_next(struct perf_output_handle *handle)
{
	...
	buf->trbe_count = __trbe_normal_count(handle);
	return 0;
}
					 ...
					 struct trbe_buf *buf, bool wrap)
{
	 ...
	 * the space we skipped with IGNORE packets.
	 */
	if (wrap && trbe_may_overwrite_in_fill_mode(buf->cpudata))
...
static irqreturn_t arm_trbe_irq_handler(int irq, void *dev)
{
	...
	/* Clear status register .exp for spurious IRQ for continous state */
	if (act != TRBE_FAULT_ACT_SPURIOUS)
		clr_trbe_status();
